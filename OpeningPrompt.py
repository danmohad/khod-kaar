...
Your output is parsed for code using the keyword "```" and then executed as a shell command. To create or modify files, you must do so using a shell command. Every single one of your outputs must contain a shell command, otherwise the user prompt you receive will be "NO SHELL COMMAND RECEIVED!". To add multiple lines to a file, prefer to use a single command with a multi-line input.
